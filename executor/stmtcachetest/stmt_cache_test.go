...
func TestCacheExecutor(t *testing.T) {
	...
	require.Equal(t, 1, executor.CacheExecManager.Size())
	tk.MustQuery("select count(*) from t1").Check(testkit.Rows(strconv.Itoa(cnt + 1)))
	tk.MustQuery("select count(*) from t1").Check(testkit.Rows(strconv.Itoa(cnt + 2)))
	tk.MustQuery("select count(*) from t1").Check(testkit.Rows(strconv.Itoa(cnt + 3)))
...
func TestCacheExecutor2(t *testing.T) {
	...
	for i := int64(0); i < totalRows; i += 1 {
		...
		tk.MustExec(fmt.Sprintf("insert into t2 values (%[1]v, %[1]v, %[2]v)", i, i%3))
	}
